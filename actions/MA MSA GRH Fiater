...
class income_object
	...
	public sub read_unea_for_hc
		...
		COLA_amount = 0.00
		'Add a section here to determine if we need the COLA (is it prior June?)'
		IF cint(maxis_footer_month) < 7 THEN
...
IF status_1619 <> "_" THEN script_end_procedure("This case has 1619 status, and the FIATer should not be used.  The script will now stop.")
...
FOR i = 0 TO ubound(income_array)
	...
	IF income_array(i).income_category = "UNEARNED" THEN
		...
		IF income_array(i).COLA_amount > 0 AND datepart("M", current_budg_month) < 7 AND income_exclusion_code <> "Y" THEN
			EMWriteScreen "X", 11, 3
			transmit
			'First, check if we already added some COLA here.
			EMReadScreen current_cola_total, 10, 14, 43
			current_cola_total = replace(current_cola_total, "_", "")
			IF IsNumeric(current_cola_total) = true Then
				new_cola_total = cint(current_cola_total) + cint(income_array(i).COLA_amount)
				EMWriteScreen "__________", 14, 43
				EMWriteScreen new_cola_total, 14, 43
			ELSE
				EMWriteScreen cint(income_array(i).COLA_amount), 14, 43
			END IF
			transmit
			PF3
		END IF
