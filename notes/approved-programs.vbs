...
call changelog_update("11/21/2019", "Banked months functionality has been updated.  The separate dialog has been removed. Enter the footer month banked months were approved for on the main dialog, and a TIKL will be set to follow up as well as case noting the banked approval. The script will only support a single month TIKL, if approving multiple months, enter only the CM+1 in the banked month fields.", "David Courtright, St. Louis")
...
End If
'Writing the TIKL for next month if BANKED months approved.  Skip TIKL if for current month

IF SNAP_banked_mo_check = checked THEN
	tikl_date = cdate(banked_footer_month & "/1/" & banked_footer_year)
	IF cdate(tikl_date) > date THEN
		call navigate_to_MAXIS_screen("DAIL", "WRIT")
		EMWRiteScreen banked_footer_month, 5, 18
		EMWRiteScreen "01", 5, 21
		EMWRiteScreen banked_footer_year, 5, 24
		call write_variable_in_TIKL("Case open to SNAP BANKED months. Review eligibility, update WREG tracking and approve for CM + 1")
		PF3
		tikled = true
	END IF
END IF
...
IF tikled = true THEN call write_variable_in_CASE_NOTE("TIKL set for " & banked_footer_month & "/01/" & banked_footer_year & " to review banked month status and update tracking.")
