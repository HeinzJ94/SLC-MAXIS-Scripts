...
call changelog_update("10/01/2019", "Initial version.", "")
...
MAXIS_footer_year = "19"
'The script----------------------------------------------------------------------------------------------------
'Connecting to MAXIS, and grabbing the case number and current footer month/year
EMConnect ""
'The message: ABAWD Waiver Ended. Member 01 is coded as 30/09.  Please review STAT/WREG coding.

'Manipulating the DAIL
member_number = mid(full_message, 28, 3) 'grab the member number from the message'

'CALL MAXIS_case_number_finder(MAXIS_case_number)
'If MAXIS_footer_month = "" Then call MAXIS_footer_finder(MAXIS_footer_month, MAXIS_footer_year)
EMConnect ""
'Writing "S" on the DAIL messagea
...
'===== Keywords: MAXIS, ABAWD, FSET, exemption, SNAP
    ...
    HH_member_array = split(member_number, " ")
    ...
    IF closing_message = "" THEN 'We will now update WREG panel'
    ...
    ELSE
    	closing_message = "*** NOTICE!!! ***" & vbCr & vbCr & "The script has checked for ABAWD and SNAP E&T exemptions coded in MAXIS for member(s) " & household_persons & "." & vbCr & closing_message & vbCr & vbCr & "Please review the case and update WREG to correct coding for exemption or counted months."
    END IF
